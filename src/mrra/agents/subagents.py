...
def make_llm(provider: str, model: str, base_url: str | None, api_key: str | None, temperature: float = 0.2, timeout: float | None = 60.0, **kwargs) -> Any:
...
def build_subagent(name: str, llm: Any, prompt: str | None, tools: List[Any]) -> Any:
    ...
    def _runner(inputs: Dict[str, Any]) -> Dict[str, Any]:
        try:
            ev_len = len(inputs.get("evidence", [])) if isinstance(inputs.get("evidence"), list) else 0
        except Exception:
            ev_len = 0
        logger.debug(f"subagent[{name}] invoking; task={inputs.get('task')} evidence_len={ev_len}")
        ...
        try:
            ...
            if not isinstance(parsed, dict) or ("selection" not in parsed and "path_ids" not in parsed):
                logger.warning(f"subagent[{name}] non-standard JSON schema keys={list(parsed) if isinstance(parsed, dict) else type(parsed)}")
                return {"_raw": parsed, "selection": [], "confidence": 0.0, "rationale": "non-standard schema"}
            ...
            if m:
                try:
                    ...
                    if not isinstance(parsed, dict) or ("selection" not in parsed and "path_ids" not in parsed):
                        logger.warning(f"subagent[{name}] JSON substring non-standard schema")
                        return {"_raw": parsed, "selection": [], "confidence": 0.0, "rationale": "non-standard schema"}
                    logger.debug(f"subagent[{name}] parsed-from-substring keys: {list(parsed.keys())}")
                    return parsed
                except Exception:
                    pass
        # 返回诊断结构，不做兜底推断
        logger.error(f"subagent[{name}] failed to parse JSON output")
        return {"_raw_text": content[:2000], "selection": [], "confidence": 0.0, "rationale": "parse_error"}
